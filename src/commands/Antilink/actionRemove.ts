...
export const command: Command = {
   name: "al.action.remove",
   description: "Remove an action for the anti link",
   category: "Antilink",
...
export const run: Run = async (client: Dash, message: Message, args: string[]) => {
   ...
   if(!serverConfig.antiLinkAction.includes(args[0].toLowerCase())) {
      return message.reply({content: `**${message.author.username}**, This action is not saved in your anti link config. If you would like to add it please use: \`${serverConfig.prefix}al.action.add ${args[0].toLowerCase()}\``})
   }
   let index;
   serverConfig.antiLinkAction.forEach((link) => {
      if(link === args[0]) {
         index = serverConfig.antiLinkAction.indexOf(link)
      }
   })
   serverConfig.antiLinkAction.splice(index)
   serverConfig.save()
   return message.reply({embeds: [
      new MessageEmbed()
      .setDescription(`\`${args[0].toLowerCase()}\` has been removed from your antilink action config.`)
        .setColor(`#EA193B`)
     ]})
}